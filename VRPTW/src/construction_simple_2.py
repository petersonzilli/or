...
class ConstructionSimple2:
    ...
    def complete_solution(self, random_seed=0):
        ...
        """
            ...
            for each customer not already used (in any order 'cause we will eval all the unused customers, so it is ok to use seed_customers)
                for each route:
        ...
        while len(seed_customers) > 0:
            ...
            #print('seed customers avaliable:      ', seed_customers)
            #print('instance customers             ', [x.number for x in self.instance.customers[1:]])
